...
If you need decrypted HTTPS request/response bodies in your traces, set up [iOS physical device network capture](/platform/ios-physical-device-network-tracing/). It's a separate one-time step per device involving a mobileconfig profile and a MITM CA trust — nothing you need for basic testing.
