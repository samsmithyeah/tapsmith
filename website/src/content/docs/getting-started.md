...
description: "Get up and running with Tapsmith in minutes. Install, configure, and write your first mobile test."
---
...
Apple Developer account, device pairing). See [iOS physical devices](/platform/ios-physical-devices/) for the full walkthrough.
...
See the [Configuration](/reference/configuration/) guide for all available options.
...
- `getByText()`, `getByRole()`, and the other `getBy*` methods are Playwright-style locators that identify UI elements. See the [Selectors Guide](/guides/selectors/) for the full list.
...
Or configure `workers` in `tapsmith.config.ts`. Each worker gets its own device. See [CI Setup](/platform/ci-setup/) for sharding across CI machines.
...
- Learn about choosing the right selectors in the [Selectors Guide](/guides/selectors/).
- Read the [Writing Tests](/writing-tests/) guide for best practices, screen objects, and test isolation.
- Browse the complete [API Reference](/reference/api/locators/).
- Configure Tapsmith for your project in the [Configuration](/reference/configuration/) guide.
- Set up [Watch and UI Mode](/guides/watch-and-ui-mode/) for fast iteration during development.
- Mock and inspect network traffic with the [Network Interception](/guides/network/) guide.
- Test hybrid apps with the [WebView Testing](/guides/webview/) guide.
- Run tests faster with the [Parallel Execution and Sharding](/guides/parallel-and-sharding/) guide.
- Set up automated testing in the [CI Setup](/platform/ci-setup/) guide.
- When things go wrong, check the [Debugging](/guides/debugging/) guide.
