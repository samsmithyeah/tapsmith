---
title: "Configuration"
description: "All tapsmith.config.ts options with defaults and examples."
---
...
| `app` | `string` | `undefined` | Path to the .app bundle under test (iOS). For simulators, build a simulator-slice `.app`. For physical devices, the `.app` must be code-signed with a profile matching the device — see [iOS physical devices](/platform/ios-physical-devices/). |
...
| `simulator` | `string` | `undefined` | iOS simulator name or UDID. Run `xcrun simctl list devices` to see available simulators. For physical iOS devices, use `device` with the UDID instead — see [iOS physical devices](/platform/ios-physical-devices/). |
