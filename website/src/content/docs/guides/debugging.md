---
title: "Debugging"
description: "Troubleshoot test failures, flaky tests, and common issues."
---
...
**Network tab** -- Displays HTTP/HTTPS requests captured during the test (when network capture is enabled). Shows method, URL, status code, duration, and response size. Click a row to see full request/response headers and bodies. See [Trace Viewer](/guides/trace-viewer/) for network capture setup details.
...
See [Configuration](/reference/configuration/#tracemode) for the complete list of trace modes.
...
This shows you exactly which selectors Tapsmith would use for each element on screen. See [MCP Server](/guides/mcp-server/) for setup.
...
See the [Selectors Guide](/guides/selectors/) for the full priority hierarchy and when to use each approach.
...
See [CI Setup](/platform/ci-setup/) for the complete workflow.
...
- An MCP server endpoint for AI coding agents (see [MCP Server](/guides/mcp-server/))
