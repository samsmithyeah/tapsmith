...
describe('findNearestHierarchy', () => {
  ...
  it('returns the nearest preceding snapshot and says which step it came from', () => {
    ...
    expect(findNearestHierarchy(hierarchies, 3)).toEqual({
      xml: '<node text="step0" />',
      sourceActionIndex: 0,
    });
  });
  ...
  it('prefers the after-snapshot over the before-snapshot at the same step', () => {
    ...
    expect(findNearestHierarchy(hierarchies, 2)).toEqual({
      xml: '<node text="after" />',
      sourceActionIndex: 1,
    });
  });
  ...
  it('never borrows forwards — only steps before the action are considered', () => {
    const hierarchies = hierarchyMap({
      'hierarchy/action-002-after.xml': '<node text="later" />',
    });

    expect(findNearestHierarchy(hierarchies, 1)).toBeUndefined();
  });

  it('returns undefined for the first action and for an empty map', () => {
    expect(findNearestHierarchy(hierarchyMap({}), 5)).toBeUndefined();
    expect(findNearestHierarchy(hierarchyMap({ 'hierarchy/action-000-after.xml': '<node />' }), 0)).toBeUndefined();
  });
});
