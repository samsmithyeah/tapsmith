/**
 * `pilot build-ios-agent` — thin xcodebuild wrapper for building the Pilot
  ...
  if (identities.length === 0) {
    throw new Error(
      'No Apple Developer team found.\n\n' +
        ...
        '    4. Re-run `pilot build-ios-agent`.',
    );
  }
...
const KNOWN_ERROR_PATTERNS: Array<{ re: RegExp; hint: (match: RegExpMatchArray) => ErrorHint }> = [
  ...
  },
  {
    ...
    hint: () => ({
      label:
        'Automatic signing could not resolve a profile. Open the project in Xcode once (ios-agent/PilotAgent.xcodeproj), select the PilotAgentUITests target, and let Xcode refresh signing.',
    }),
  },
  {
  ...
  },
  {
    ...
    hint: () => ({
      label:
        'Profile was built but iOS refused installation. On the device, Settings → General → VPN & Device Management → trust the Pilot developer profile.',
    }),
  },
  {
    ...
    hint: () => ({
      label:
        'PilotAgent requires an entitlement your current profile does not include. This is unusual for the Pilot agent — file an issue with the full xcodebuild output.',
    }),
  },
  {
...
export async function buildIosAgent(options: BuildIosAgentOptions): Promise<string> {
  ...
  if (!fs.existsSync(path.join(iosAgentDir, 'PilotAgent.xcodeproj'))) {
    throw new Error(
      `ios-agent/PilotAgent.xcodeproj not found under ${iosAgentDir}.\n` +
        '  Run this command from the Pilot repo root, or pass a different --cwd.',
    );
  }
  ...
  const args = [
    'build-for-testing',
    '-project', path.join(iosAgentDir, 'PilotAgent.xcodeproj'),
  ...
  console.log(bold('Building PilotAgent runner for iOS devices…'));
  ...
  if (!ok) {
    ...
    throw new Error(`pilot build-ios-agent failed (${elapsedSec}s)`);
  }
 ...
 * a staging directory (e.g. `/tmp/PilotAgent.dst`). Xcode 26+ interprets that
...
/**
 * Argument parser for `pilot build-ios-agent`. Kept deliberately small —
...
function printHelp(): void {
  console.log(`
${bold('pilot build-ios-agent')} — Build the signed PilotAgent XCUITest runner for physical iOS devices.

${bold('Usage:')}
  pilot build-ios-agent [options]

${bold('Options:')}
  --team-id <TEAMID>     Apple Developer team ID (auto-detected if omitted)
  --cwd <path>           Path to Pilot repo root (default: cwd)
...
export async function runBuildIosAgent(argv: string[]): Promise<void> {
  if (process.platform !== 'darwin') {
    console.error(red('pilot build-ios-agent is only supported on macOS.'));
    process.exit(1);
  }
