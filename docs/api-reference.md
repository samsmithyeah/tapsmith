...
- **Android**: archives the app's data directory (`/data/data/<package>/`) — SharedPreferences, databases, and internal files. Requires root (emulators) or a debuggable app (`run-as` fallback on physical devices). Auth state that lives in SharedPreferences (e.g. `@react-native-firebase/auth`'s default token persistence) round-trips fine. **Limitation:** values protected by Keystore-backed encryption (`EncryptedSharedPreferences`, `expo-secure-store`) store only their *ciphertext* in the data directory; the decryption key lives in the Android Keystore, which is hardware/TEE-backed and **not** captured. Because `restoreAppState` runs `pm clear` (which destroys the app's Keystore keys) before extracting, such values cannot be decrypted after a restore. There is no general fix (Keystore keys are non-exportable by design) — prefer API-driven auth for setup when an app uses Keystore-encrypted persistence.
