...
| `rootDir` | `string` | the config file's directory | Root for test discovery. Relative values are resolved against the config file's directory, so a config behaves the same however it is loaded (`-c ../other/tapsmith.config.mjs`, an MCP server started elsewhere). Falls back to the working directory when there is no config file. |
