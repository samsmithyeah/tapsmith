...
| `devices` | `number \| { name: string; device?: string }[]` | `undefined` | Drive several devices from one test — `devices: 2`, or named members optionally pinned to a serial. Project-level only. Tests receive them as the `devices` fixture. See [Multi-device tests](./multi-device.md). |
